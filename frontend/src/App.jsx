...
const initialTelemetry = { temperature: 0, vibration: 0, current: 0, acceleration: 0, anomaly_logit: 0, anomaly_probability: 0, anomaly_detected: false, processing_time_ms: 0 };
const streams = [["TEMP-01", "Temperature"], ["VIB-02", "Vibration"], ["CURR-03", "Current"], ["ACC-04", "Acceleration"], ["TEMP-05", "Temperature"], ["VIB-06", "Vibration"], ["CURR-07", "Current"], ["ACC-08", "Acceleration"]];
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket.onmessage = (event) => {
        ...
        } catch { console.error("Invalid telemetry message"); }
      };
      socket.onclose = () => { setConnected(false); if (!disposed) timer = window.setTimeout(connect, 2000); };
  ...
  return (
    <main className="app-shell">
      ...
      <header className="topbar">
        <div className="brand-block"><div className="brand-mark">ES</div><div><div className="brand-row"><h1>EdgeSense</h1><span className="version">EDGE AI</span></div><p>Real-time sensor fusion &amp; anomaly detection</p></div></div>
        <div className="system-state"><span className={`state-dot ${connected ? "online" : "offline"}`} /><div><strong>{connected ? "LIVE PIPELINE" : "WAITING FOR PIPELINE"}</strong><span>{connected ? "WebSocket :9002" : "Reconnecting automatically"}</span></div></div>
      </header>

      <section className="hero-grid">
        <article className={`hero-card ${anomaly ? "danger" : "healthy"}`}>
          <div className="hero-copy"><div className="section-kicker">MODEL DECISION</div><div className="hero-status-row"><span className={`status-icon ${anomaly ? "danger" : "healthy"}`}>{anomaly ? "!" : "✓"}</span><h2>{anomaly ? "ANOMALY DETECTED" : "SYSTEM NORMAL"}</h2></div><p>ONNX Runtime is evaluating the synchronized sensor window in real time.</p></div>
        ...
        <article className="architecture-card"><div className="section-kicker">PIPELINE</div><div className="pipeline-flow"><PipelineNode label="8 STREAMS" detail="C++20" /><i>→</i><PipelineNode label="FUSION" detail="Buffered" /><i>→</i><PipelineNode label="INFERENCE" detail="ONNX" /><i>→</i><PipelineNode label="TELEMETRY" detail="WebSocket" /></div><div className="architecture-footer"><span>React dashboard</span><span>•</span><span>Live monitoring</span></div></article>
      </section>

      <section className="metric-grid"><Metric label="TEMPERATURE" value={telemetry.temperature} unit="°C" /><Metric label="VIBRATION" value={telemetry.vibration} unit="g" /><Metric label="CURRENT" value={telemetry.current} unit="A" /><Metric label="ACCELERATION" value={telemetry.acceleration} unit="g" /><Metric label="INFERENCE" value={latency} unit="ms" accent="purple" /></section>
      ...
      <section className="dashboard-grid">
        ...
        <article className="panel inference-card"><PanelHeader kicker="ML INFERENCE" title="Anomaly model" meta="PyTorch → ONNX Runtime" /><div className="model-score"><div className="score-ring" style={{ "--score": `${probability * 100}%` }}><div><strong>{(probability * 100).toFixed(0)}%</strong><span>confidence</span></div></div></div><Detail label="Logit" value={(Number(telemetry.anomaly_logit) || 0).toFixed(3)} /><Detail label="Inference latency" value={`${latency.toFixed(2)} ms`} /><Detail label="Peak observed latency" value={`${maxLatency.toFixed(2)} ms`} /><Detail label="Input features" value="4 sensor features" /></article>
      </section>

      <section className="lower-grid">
        <article className="panel stream-panel"><PanelHeader kicker="SENSOR FABRIC" title="8-stream workload" meta="Concurrent simulated inputs" /><div className="stream-grid">{streams.map(([name, type]) => <div className="stream" key={name}><span className={`stream-indicator ${connected ? "active" : ""}`} /><div><strong>{name}</strong><span>{type}</span></div><b>{connected ? "ACTIVE" : "IDLE"}</b></div>)}</div></article>
        <article className="panel health-panel"><PanelHeader kicker="RUNTIME HEALTH" title="Pipeline status" meta={lastUpdate ? `Updated ${lastUpdate.toLocaleTimeString()}` : "No telemetry yet"} /><Health label="WebSocket transport" value={connected ? "Connected" : "Offline"} healthy={connected} /><Health label="8-stream ingestion" value={connected ? "Running" : "Waiting"} healthy={connected} /><Health label="ML inference" value={connected ? "Online" : "Standby"} healthy={connected} /><Health label="Anomaly state" value={anomaly ? "Alert" : "Normal"} healthy={!anomaly} /></article>
      </section>
